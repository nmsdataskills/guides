...
`cars` only has the `speed`args = commandArgs(trailingOnly=TRUE)
lines = strsplit(readLines(args[[1]], warn=FALSE), '\n')
for (line in lines) {
  if (length(line)) {
    strings = strsplit(line, ' ')
    numbers = sapply(strings, as.integer)
    x = numbers[[1]]
    y = numbers[[2]]
    n = numbers[[3]]
    for (i in (1:n)){
...
} and `dist` columns available in `newcars`.
